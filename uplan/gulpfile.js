...
gulp.task('dist', ['clean'], function() {
	gulp.start('custom-js', 'sass', 'html', 'js', 'fonts', 'images', "json", "uplan", "plugIn");
});
...
gulp.task('uplan', function() {
	return gulp.src(src.uplan)
		.pipe($.plumber())
...
gulp.task('sass', function() {
	return gulp.src([src.sass, '!' + src.uplan])
		.pipe($.plumber())
		.pipe($.sourcemaps.init())
		.pipe($.sass({ outputStyle: 'expanded' }).on('error', $.sass.logError))
		.pipe($.autoprefixer(autoprefixer))
...
gulp.task('watch', function() {
	...
	gulp.watch(src.sassTools, ['uplan']);
