+ function($) {
    ...
    var Page = function(element, options) {
        ...
        this.$page = $('<ul class="pagination-page"></ul>').hide();
        this.$size = $('<div class="pagination-size"></div>').hide();
        this.$jump = $('<div class="pagination-jump"></div>').hide();
        this.$info = $('<div class="pagination-info"></div>').hide();
    ...
    Page.prototype = {
        ...
        setPageSize: function(pageSize) {
            if (pageSize !== undefined && pageSize !== null) {
                ...
                var $select=this.$size.find("select");
                $select.val(pageSize)
                if($select[0].selectedIndex<0){
                    $select.prepend('<option value="'+pageSize+'">'+pageSize+'</option>');
                }
            }
        },
        setOption: function(option) {
            if (typeof option == "object") {
                for (var i in option) {
                    ...
                    if (i == 'pageIndex') {
                        this.setPageIndex(option[i])
                    } else if (i == "pageSize") {
                        this.setPageSize(option[i])

                    } else if (i == 'params') {
                        this.setParams(option[i])
                    } else if (i == "url") {
                        this.setRemoteUrl(option[i])
                    } else {
                        this.options[i] = option[i];
                    }
        ...
        initHtml: function() {
            ...
            //init jump module
            var jumpHtml = '<div class="pagination-group"><input data-page-btn="jump" type="text"><button data-page-btn="jump" type="button">' + this.options.jumpBtnText + '</button></div>';
        ...
        remote: function() {
            ...
            pagination.remote.getAjax(this, this.currentUrl, this.currentParams, this.ajaxCallBack, this.options.remote.beforeSend, this.options.remote.complete, this.options.remote.type);
        },
        ...
        renderPagination: function() {
            var option = {
                ...
                alwaysBtnShow: this.options.alwaysBtnShow,
    ...
    pagination.remote = {
        getAjax: function(pagination, url, data, success, beforeSend, complate, type) {
            $.ajax({
                type: type,
                url: url,
                dataType: 'json',
    ...
    pagination.core = {
        ...
        */
        renderPages: function(pageIndex, pageSize, total, pageBtnCount, options) {
            ...
            var prevPage = this.renderPerPage(options.prevBtnText, ((pageIndex - 1) < 0 ? pageIndex : (pageIndex - 1)));
            ...
            if (options.alwaysBtnShow) {


                behindBtnNum = lastPageNumber - ((pageBtnCount - 2) / 2);


                if (lastPageNumber <= pageBtnCount - 4) {

                    html = this.renderGroupPages(1, lastPageNumber, pageNumber);

                } else {
                    if (pageNumber <= symmetryBtnCount) {

                        html = this.renderGroupPages(1, pageBtnCount - 4, pageNumber);



                    } else if (pageNumber > behindBtnNum + symmetryBtnCount) {

                        html = this.renderGroupPages(lastPageNumber - pageBtnCount + 5, pageBtnCount - 4, pageNumber);
